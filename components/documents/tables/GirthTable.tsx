...
export const GirthTable: React.FC<GirthTableProps> = ({
  ...
  // Track which measurement input is showing suggestions
  const [activeMeasurementInput, setActiveMeasurementInput] = useState<string | null>(null);
                  ...
                  <td className="px-2 py-2">
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => {
                          onUpdateEntry(entry.id, { measurement: e.target.value });
                          if (e.target.value.length > 0) {
                            setActiveMeasurementInput(entry.id);
                          }
                        }}
                        onFocus={() => {
                        ...
                        onBlur={() => {
                          ...
                          setTimeout(() => setActiveMeasurementInput(null), 200);
                        }}
                      ...
                      />
                      {activeMeasurementInput === entry.id && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-cairos-border rounded-lg shadow-lg z-20 max-h-60 overflow-y-auto">
                          {measurementOptions
                            .filter(opt => {
                              const searchValue = (entry.measurement || '').toLowerCase();
                            ...
                            .map((option) => (
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  onUpdateEntry(entry.id, { measurement: option.label });
                          ...
                          {measurementOptions.filter(opt => {
                            const searchValue = (entry.measurement || '').toLowerCase();
                            return opt.label.toLowerCase().includes(searchValue) && opt.value !== '';
                          }).length === 0 && (
                            <div className="px-3 py-2 text-body-xs text-gray-500">
                              Type to see suggestions or enter custom measurement
                            </div>
                          )}
                        </div>
                      )}
